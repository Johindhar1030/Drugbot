...
def handle_chat_message(
    ...
    # 3. Controlled Query Normalization Layer (BEFORE retrieval / safety / resolution)
    state = memory.get_state(session_id, user_id=user_id)
    has_context = bool(state.get("last_question") or memory.get_history(session_id, user_id=user_id) or active_drug)
    norm_result = normalize_query(
        message,
        session_id=session_id,
        has_conversation_context=has_context,
        user_id=user_id,
    )
    effective_query = norm_result.normalized_query if (norm_result.confidence >= 0.70 and not norm_result.needs_clarification) else message

    # Handle ambiguous / incomplete query requiring clarification (zero LLM cost)
    if norm_result.needs_clarification:
        clarification_reply = norm_result.clarification_message or f'Could you please clarify what you mean by "{message.strip()}"?'
        memory.add_turn(session_id, "user", message, user_id=user_id)
        ...
        return {
            ...
            "normalization_info": norm_result.to_dict(),
        }

    # 4. Safety classification — regex first (free), LLM fallback
    classification = classify_question(effective_query, llm=llm)
    logger.info(
    ...
    # 5. High-risk shortcut — no retrieval, no generation
    ...
    # 5b. Out-of-scope shortcut — gentle empathetic refusal + privacy-preserving telemetry
    ...
    # 6. Context Resolver / Query Rewriting
    resolved = resolve_context(effective_query, state, llm=llm, user_id=user_id)
    logger.info(
    ...
    # Handle ambiguous query from context resolver
    ...
    from app.retrieval.vector_store import normalize_drug_name, normalize_drug_names_list, get_known_drugs
    target_drugs = []
    all_user_drugs = get_known_drugs(user_id=user_id)

    # Handle multi-drug intent: entities["drugs"] is a list
    if resolved.intent == "multi_drug" and isinstance(resolved.entities.get("drugs"), list):
        target_drugs = normalize_drug_names_list(resolved.entities["drugs"], user_id=user_id)
    else:
        resolved_drug = resolved.entities.get("drug") or resolved.entities.get("drugs")
        if isinstance(resolved_drug, (list, tuple)):
            target_drugs = normalize_drug_names_list(list(resolved_drug), user_id=user_id)
        elif resolved_drug:
    ...
    # If no specific drug targeted, search across ALL of the user's uploaded drugs
    # so every uploaded PDF is always considered for every query
    if target_drugs:
        retrieval_drug_filter = target_drugs
    elif active_drug:
        retrieval_drug_filter = active_drug
    elif all_user_drugs:
        retrieval_drug_filter = all_user_drugs
    else:
        retrieval_drug_filter = None
    ...
    num_drugs_in_query = len(target_drugs) if target_drugs else (len(all_user_drugs) if not active_drug else 1)
    context_chunks = select_context_chunks(resolved.resolved_query, top_chunks, num_drugs=num_drugs_in_query)
    ...
    if resolved.intent == "comparison":
        gen_mode = "comparison"
    elif resolved.intent == "multi_drug":
        gen_mode = "multi_drug"
