...
_MULTI_DRUG_PROMPT = """\
Answer from the context ONLY. Do not use outside knowledge.
The user is asking about multiple drugs. Provide a separate section for EACH drug mentioned.

Rules:
(1) Use a bold header for each drug: **DRUG NAME**
(2) Under each drug, summarize: indications/usage, key warnings, and any other relevant info present in the context.
(3) Cite [chunk_N] after each fact. Every factual statement MUST end with a [chunk_N] citation.
(4) If information for a drug is not in the context, state: "No information found in the provided documents for [DRUG NAME]."
(5) Do NOT mix information between drugs.
...
Answer (one section per drug, with [chunk_N] citations):"""
...
def generate_answer(
    ...
    if mode == "multi_drug":
        template = _MULTI_DRUG_PROMPT
    elif mode == "comparison" or re.search(r"\b(table|structured table|comparison table|compare in a table|compare)\b", question, re.I):
