...
def _classify_query_complexity(query: str, num_drugs: int = 1) -> str:
    """Classify query complexity to set context budget scaling."""
    q = query.lower()
    # Complex: comparisons, tables, multiple indications, or multiple drugs
    if num_drugs > 1 or any(w in q for w in ("compare", "versus", "vs", "difference", "table", "all indications")):
        return "complex"
...
def select_context_chunks(
    ...
    complexity = _classify_query_complexity(query, num_drugs=num_drugs)
    ...
    # For multi-drug queries, guarantee at least 1 chunk per drug is included
    if num_drugs > 1:
        selected_ids = {id(c) for c in selected}
        by_drug: dict[str, list[dict]] = {}
        for c in chunks:
            d = c.get("metadata", {}).get("drug_name", "")
            by_drug.setdefault(d, []).append(c)
        for drug_chunks in by_drug.values():
            if not any(id(c) in selected_ids for c in drug_chunks):
                selected.append(drug_chunks[0])
                selected_ids.add(id(drug_chunks[0]))
                used_tokens += _estimate_tokens(drug_chunks[0]["text"])
