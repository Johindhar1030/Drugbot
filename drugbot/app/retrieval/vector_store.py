...
def normalize_drug_name(name: str | list | tuple | None, user_id: int | None = None) -> str | None:
    ...
    if isinstance(name, (list, tuple)):
        ...
        if not valid_items:
            return None
        # Normalize each and return the first valid one
        for item in valid_items:
            result = normalize_drug_name(item, user_id=user_id)
            if result:
                return result
        return None
...
def normalize_drug_names_list(names: list | None, user_id: int | None = None) -> list[str]:
    """Normalize a list of drug name strings, returning all valid matches."""
    if not names:
        return []
    result = []
    for name in names:
        nd = normalize_drug_name(name, user_id=user_id)
        if nd and nd not in result:
            result.append(nd)
    return result
